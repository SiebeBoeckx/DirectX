...
namespace dae
{
	...
	Mesh_PosTex::Mesh_PosTex(ID3D11Device* pDevice, std::vector<Vertex_PosTex> vertices, std::vector<uint32_t> indices, const Matrix& worldMatrix, Texture* pTexture, Texture* pNormal, Texture* pSpecular, Texture* pGloss)
		:m_pDevice{ pDevice }
		, m_VerticesTex{ vertices }
		, m_Indices{ indices }
		, m_WorldMatrix{ worldMatrix }
	{
		m_pEffect = new Effect_PosTex{ m_pDevice, std::wstring{L"./Resources/PosCol3D.fx"} };
		m_pTechnique = m_pEffect->GetTechnique();
	...
	void Mesh_PosTex::Render(ID3D11DeviceContext* pDeviceContext, const Camera& camera)
	{
		//1. Set primitive topology
		pDeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	...
	void Mesh_PosTex::CycleSamplerState()
	{
